...
// ── Banner Play Store — 1 nave arrastando bandeira PNG com física ─────────────
...
function _pickUniqueSkins(n) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (!pool.length) pool.push(...SKINS);
  }
  return out;
}
...
function _initBannerShips(W, H) {
  ...
  const speed = (36 + Math.random() * 16) * dir;
  const cols  = _BFLAG_COLS;
  ...
  const offVY = new Float32Array(cols + 1).fill(0);

  return {
    dir, speed, y, skin, BW, BH, cols,
...
// Desenha a bandeira PNG fatiada em colunas com deslocamento Y por coluna
function _drawFlagPNG(ctx, bs, t) {
...
function _tickBannerShips(ctx, W, H, dt, t) {
  ...
  // Bandeira PNG com física de ondulação
  _drawFlagPNG(ctx, bs, t);
...
let _bannerLastSpawnMs = -99999; // controla intervalo entre banners (ms desde abertura)
let _bannerPrevT = 0;
...
(function animateLoginBg(){
  try {
    ...
    if (SKINS.length) {
      ...
      // Aparece 4s após abrir a página, e repete a cada 35s após terminar
      if (!_loginBgState.bannerShips && nowMs - _bannerLastSpawnMs > 4000) {
        _loginBgState.bannerShips = _initBannerShips(W, H);
        _bannerLastSpawnMs = nowMs;
      }
      if (_loginBgState.bannerShips) {
        _tickBannerShips(ctx, W, H, dt, t);
        if (_loginBgState.bannerShips.done) {
          _loginBgState.bannerShips = null;
          _bannerLastSpawnMs = performance.now();
        }
      }
    }
...
function showScreen(name){
  ...
  if (name !== 'login' && music) {
    music.pause();
    music.currentTime = 0; // reseta para não vazar som ao voltar ao login
  }
